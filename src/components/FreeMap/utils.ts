...
export const toMapCoord = (p: LatLng): MapCoord => [p.lng, p.lat];

export const fromMapCoord = (coord: number[]): LatLng => ({
    lat: coord[1],
    lng: coord[0],
});

export const getPointsFromProps = (props: any): IncyclistNode[] => {
    const validPoint = (p: any) =>
        p && (p.lat !== undefined || p.latitude !== undefined) &&
        (p.lng !== undefined || p.longitude !== undefined);

    const normalize = (p: any): IncyclistNode => ({
        lat: p.lat ?? p.latitude,
        lng: p.lng ?? p.longitude,
        routeDistance: p.routeDistance ?? p.distance,
    });

    if (props.points?.length) return props.points.filter(validPoint).map(normalize);
    if (props.route?.details?.points?.length) return props.route.details.points.filter(validPoint).map(normalize);
    if (props.activity?.logs?.length) {
        return props.activity.logs.map((log: any) => ({
            lat: log.lat,
            lng: log.lng,
            routeDistance: log.distance,
        })).filter(validPoint);
    }
    return [];
};
 ...
 * Skips points where lat or lng is undefined.
...
export const computeBoundsFromPoints = (
    ...
    const validPoints = allPoints.filter((p) => p && p.lat !== undefined && p.lng !== undefined);
